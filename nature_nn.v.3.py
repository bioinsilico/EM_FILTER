...
from globa_micrograph2np import micrograph2np
...
micrograph2np(n,int(alpha*n))

model = keras.models.Sequential()

input_dim = n
input_conv_dim = int(alpha*n)

dim_net_FC2 = int(alpha*alpha*n*n)
dim_net_FC3 = int(alpha*alpha*n*n)
...
#C2
model.add( keras.layers.Conv2D(filters=64, kernel_size=(5,5), strides=(1, 1), padding='same', activation='relu', activity_regularizer=keras.regularizers.l1(l1_reg) ))
...
n1 = int(0.5*n*(1-alpha))#int(n*0.25)
n2 = int(0.5*n*(1+alpha))##int(n*0.75)
